...
public class JourneyResource {
    ...
    @GetMapping("/journeys")
    @Timed
    public ResponseEntity<List<JourneyDTO>> getAllJourneys(@ApiParam Pageable pageable) {
    ...
    @Timed
    public ResponseEntity<List<JourneyDTO>> searchJourneys(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of Journeys for query {}", query);
        Page<JourneyDTO> page = journeyService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/journeys");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
